...
WordPress integration is in progress. Its native PHP assessment engine matches complete Python results across 9,253 valid/malformed inputs under PHP 8.3. It adds no Python or remote-service requirement to a prospective WordPress host. It is not yet an installable plugin: publishing hooks, authenticated evidence capture, version storage, notices and actual WordPress integration tests remain required. See `integrations/wordpress/README.md`.
