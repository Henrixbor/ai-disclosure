...
The demo, media and WordPress runners accept `AI_DISCLOSURE_BROWSER=chromium|firefox|webkit` (default Chromium). Run each engine, for example `AI_DISCLOSURE_BROWSER=firefox npm run test:browser` and `AI_DISCLOSURE_BROWSER=webkit node scripts/browser_media.cjs`. CI runs all three against the same generated fixtures; an unknown engine is an error. Screenshots include the engine name. These are Playwright browser builds and desktop contexts at different viewport widths, not certification of installed Safari/Chrome, physical mobile devices or assistive technologies. Both WordPress runners also accept this selector, for example `AI_DISCLOSURE_BROWSER=firefox npm run test:wordpress:mysql`. CI runs their editor and notice checks in all three engines, including the MySQL admin replacement flow. The Next.js integration runner currently uses Chromium.
