...
The [experimental WordPress text plugin](integrations/wordpress/README.md) runs a native PHP engine with private evidence recording and publication gates. Classic and block editor controls let publishers assess current text, amend recorded facts and withdraw invalid draft assessments. Agents can inventory existing posts and pages through a private paginated endpoint before migrating established facts. Its live WordPress tests run on SQLite and MySQL 8.4 and cover the documented editor/REST/native update paths, inventory permissions and text notices. It is intended for isolated integration testing while broader content workflows remain incomplete.
