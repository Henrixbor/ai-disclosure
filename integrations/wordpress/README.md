...
The MySQL runner also starts separate PHP workers with distinct database connections and coordinates them at actual write boundaries. It verifies that competing amendments yield exactly one winner and one 409 while preserving the original audit record. It exercises both withdrawal/publication orderings: a withdrawal committed after publication precheck returns the final post to draft with anonymous HTTP access denied; publication committed before the withdrawal write makes the draft guard reject withdrawal. These are deterministic interleaving tests, not a load test or proof that no reader can observe an intermediate state. External page caches and other publishing paths remain separate deployment checks.
