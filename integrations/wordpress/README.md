...
From the repository root, install development dependencies with `npm ci --ignore-scripts`, install the browser engines with `npx playwright install chromium firefox webkit`, then run `npm run test:wordpress`. Playground boots a disposable WordPress 7.1/PHP 8.3 site, activates the actual plugin, exercises native/REST operations and verifies public output with JavaScript disabled (Chromium by default). It also logs into disposable admin credentials to exercise the classic and block editor panels, stale text, recording, review amendments, draft withdrawal/restoration and actual editor publication. Inventory tests cover bounded traversal, permission filtering, fixed upper IDs, legacy unknown-origin publications, withdrawn/unsupported content and database failures. It closes the test server afterward. Production HTTPS/application-password configuration, other roles, themes and editor versions still need deployment-specific verification.
...
Both runners accept `AI_DISCLOSURE_BROWSER=chromium|firefox|webkit`; for example, `AI_DISCLOSURE_BROWSER=webkit npm run test:wordpress:mysql`. CI runs each engine against an independent disposable fixture. These desktop browser contexts cover the documented editor and notice scenarios, not physical mobile devices, assistive technologies or every installed browser version.
